...
async function buildRegistry(registry: Registry) {
 ...
 `
   ...
   for (const style of styles) {
      ...
      for (const item of registry) {
         const isSpecial = item.type === 'components:special'
         const resolveFiles = item.files.map((file) => {
            if (file.startsWith('special/'))
               return `registry/special/${file.replace('special/', '')}`

            return `registry/${style.name}/${file}`
         })
         ...
         if (item.type === 'components:block') {
            ...
            // Find all imports.
            const imports = new Map<string, { module: string, text: string, isDefault?: boolean }>()
            sourceFile.getImportDeclarations().forEach((node) => {
               const module = node.getModuleSpecifier().getLiteralValue()
               node.getNamedImports().forEach((item) => {
                  imports.set(item.getText(), { module, text: node.getText() })
               })

               const defaultImport = node.getDefaultImport()
               if (defaultImport)
                  imports.set(defaultImport.getText(), { module, text: defaultImport.getText(), isDefault: true })
            })

            // Find all opening tags with x-chunk attribute.
            const components = sourceFile
               .getDescendantsOfKind(SyntaxKind.JsxOpeningElement)
               .filter(node => node.getAttribute('x-chunk') !== undefined)
            ...
            chunks = await Promise.all(
               components.map(async (component, index) => {
                  ...
                  component.addAttribute({ name: 'x-chunk', initializer: `"${chunkName}"` })
                  ...
                  children.forEach((child) => {
                     ...
                     if (importLine) {
                        ...
                        const newImports = importLine.isDefault ? importLine.text : new Set([...existingImports, child])
             ...
             export default function Component() {
               return (${parentJsxElement.getText()});
                  ...
                  const targetFilePath = path.join(cwd(), `registry/${isSpecial ? 'special' : style.name}/${type}/${chunkName}.tsx`)
                  ...
                  return {
                     name: chunkName,
                     description,
                     component: `React.lazy(() => import("~/registry/${isSpecial ? 'special' : style.name}/${type}/${chunkName}"))`,
                     file: targetFile,
         ...
         const importPath = item.files[0].startsWith('special/')
            ? `~/registry/special/${item.files[0].replace('special/', '').replace(/\.tsx$/, '')}`
            : `~/registry/${style.name}/${item.files[0].replace(/\/ui\//, '/').replace(/\.tsx$/, '')}`
 ...
 `
   ...
   // ----------------------------------------------------------------------------
   const names = registry.filter(item => item.type === 'components:ui' || item.type === 'components:special')
