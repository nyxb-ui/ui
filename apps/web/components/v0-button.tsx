...
import { Button, type ButtonProps } from "~/registry/miami/ui/button"
import {
   Tooltip,
   TooltipContent,
   TooltipTrigger,
...
export function V0Button({
   ...
   const [url, setUrl] = React.useState("https://ui.shadny.com")
...
export function V0Logo({ className, ...props }: React.ComponentProps<"svg">) {
   return (
      <svg
         ...
         className={ny("h-5 w-5 text-current", className)}
         {...props}
      >
         <path
