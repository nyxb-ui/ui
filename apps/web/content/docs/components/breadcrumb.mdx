...
```bash
npx nyxbui@latest add breadcrumb
```
...
To use a custom link component from your routing library, you can use the `asChild` prop <br /> on `<BreadcrumbLink />`.
...
Here's an example of a responsive breadcrumb that composes `<BreadcrumbItem />` <br /> with `<BreadcrumbEllipsis />`, `<DropdownMenu />`, and `<Drawer />`.
