...
```bash
npm install tailwindcss-animate class-variance-authority clsx tailwind-merge framer-motion
```
...
If you're using the `miami` style, install `@radix-ui/react-icons`:
...
I use a `ny` helper to make it easier to conditionally add Tailwind CSS classes. Here's how I define it in `lib/utils.ts`:
