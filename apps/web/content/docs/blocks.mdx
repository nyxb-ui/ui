...
```bash
git clone https://github.com/nyxb-ui/ui.git
```
...
Create a new folder in the `apps/web/registry/miami/blocks` directory. Make sure the folder is named in kebab-case and under `miami`.
...
This follows the registry schema at [https://nyxbui.design/schema/registry-item.json](https://nyxbui.design/schema/registry-item.json).
...
Once the build script is finished, you can view your block at `http://localhost:3333/blocks/[CATEGORY]` or a full screen preview at `http://localhost:3333/view/styles/miami/dashboard-01`.
...
**Note:** If you've run the capture script before, you might need to delete the existing screenshots (both light and dark) at `apps/www/public/r/styles/miami` and run the script again.
...
If you need to add a new category, you can do so by adding it to the `registryCategories` array in `apps/web/registry/registry-categories.ts`.

```tsx title="apps/web/registry/registry-categories.ts"
...
- **Imports should always use the `~/registry` path.** eg. `import { Input } from "~/registry/miami/input"`
