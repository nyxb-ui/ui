...
You can see the JSON Schema for `registry-item.json` [here](https://nyxbui.design/schema/registry-item.json).
...
- For `nyxb/ui` registry items such as `button`, `input`, `select`, etc use the name eg. `['button', 'input', 'select']`.
...
By default, the `nyxb` cli will read a project's `components.json` file to determine the target path. For some files, such as routes or config you can specify the target path manually.
