...
If you're starting a new registry project, you can use the [registry template](https://github.com/nyxb-ui/registry-template) as a starting point. We have already configured it for you.

## registry.json

The `registry.json` file is only required if you're using the `nyxb` CLI to build your registry.
...
Note: the `build` command is currently only available in the `nyxb@canary` version of the CLI.

```bash
npm install nyxb@canary
```
...
You can change the output directory by passing the `--output` option. See the [nyxb build command](/docs/cli#build) for more information.
...
To install a registry item using the `nyxb` CLI, use the `add` command followed by the URL of the registry item.

```bash
npx nyxb@latest add http://localhost:3000/r/hello-world.json
```
