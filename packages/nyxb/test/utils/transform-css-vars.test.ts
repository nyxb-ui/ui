...
export function Foo() {
  ...
  expect(
    await transform({
      ...
      raw: `import * as React from "react"
export function Foo() {
	return <div className={ny("bg-background hover:bg-muted", true && "text-primary-foreground sm:focus:text-accent-foreground")}>foo</div>
}"
    `,
      config: {
        tsx: true,
  ...
  expect(
    await transform({
      ...
      raw: `import * as React from "react"
export function Foo() {
	return <div className={ny("bg-background border border-input")}>foo</div>
}"
    `,
      config: {
        tsx: true,
