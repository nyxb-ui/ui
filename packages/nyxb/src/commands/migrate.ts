   ...
   .action(async (migration, opts) => {
      try {
         ...
         if (
            ...
            throw new Error(
               'No `nyxbui.json` file found. Ensure you are at the root of your project.',
            )
         }

         if (!config) {
            throw new Error(
               'Something went wrong reading your `nyxbui.json` file. Please ensure you have a valid `nyxbui.json` file.',
            )
         }
